...
const sslOptions = {
	key: fs.readFileSync(process.env.SSL_KEY_PATH || '/app/certs/social.key'),
	cert: fs.readFileSync(process.env.SSL_CERT_PATH || '/app/certs/social.crt'),
...
https.createServer(sslOptions, apiApp).listen(API_PORT, () => {
	console.log(`Social API (HTTPS) listening on ${API_PORT}`);
});

https.createServer(sslOptions, metricsApp).listen(METRICS_PORT, () => {
  console.log(`Social Metrics (HTTPS) listening on ${METRICS_PORT}`);
});